...
public class FileInputStreamTest {
	...
	// 찍는지 확인하는 용도로 true로 하는것. 출력 안하려면 false
	private static boolean bPrint = false;
	
	public static long test01() {
		
		long start = System.currentTimeMillis();
		// 부모는 자식을 받을 수 있음. InputStream이 상위 클래스. FileInputStream로 만들었어도 InputStream으로 받을 수 있는 이유
		try (InputStream is = new FileInputStream("icudtl.dat");) {
			int ch;
			while(0 <= (ch = is.read())) {
				if (bPrint)
	...
	public static long test03() {
		// BufferedInputStream이 제일 속도가 빠르다 
		
		byte[] buffer = new byte[1024];

		long start = System.currentTimeMillis();
		try (InputStream bis = new BufferedInputStream(new FileInputStream("icudtl.dat"));) {
	...
	public static long test04() {
		...
		long start = System.currentTimeMillis();
		try (Reader fr = new FileReader("icudtl.dat");) {
			int ch;
			while(0 <= (ch = fr.read())) {
				if (bPrint)
					System.out.println(ch);
	...
	public static long test06() {
		...
		long start = System.currentTimeMillis();
		try (Reader br = new BufferedReader(new FileReader("icudtl.dat"));) {
	...
	public static long test07() {
		...
		long start = System.currentTimeMillis();
		try (RandomAccessFile raf = new RandomAccessFile("icudtl.dat", "r");
